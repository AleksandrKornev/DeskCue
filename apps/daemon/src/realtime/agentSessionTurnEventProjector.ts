...
function shouldSuppressManagedAttachedTurn(
  ...
  if (previousOwner !== "managed" && owner !== "managed") return false;
...
function shouldPublishTurnFinishedEvent(
  ...
  if (previous.state.phase === "active") return true;
...
function findLinkedManagedSessionId(
  application: DaemonApplication,
  agentSession: AgentSessionDetail
) {
  const linkedSessions = application.managedSessions.listSessions().filter(
    (session) =>
      session.adapterId === agentSession.agentId &&
      session.sourceSessionId === agentSession.sourceSessionId
  );

  return linkedSessions.length === 1 ? linkedSessions[0].id : null;
}
...
function findNewTerminalTurns(
  ...
  if (!Number.isFinite(previousObservedAt)) return [];
...
function findLatestTurnAnswer(
  ...
  for (let index = end - 1; index >= start; index -= 1) {
    ...
    if (text) return truncateNotificationAnswer(text);
  }
...
function findTurnStartedAt(
  ...
  for (let index = end - 1; index >= 0; index -= 1) {
    ...
    if (label === "Turn started") return entry.timestamp;
  }
...
function findTurnDurationMs(
  ...
  if (completedMatch?.[1]) return Number(completedMatch[1]) * 1000;

  const interruptedMatch = detail.match(/\bafter (\d+)s\b/);

  if (interruptedMatch?.[1]) return Number(interruptedMatch[1]) * 1000;
...
export class AgentSessionTurnEventProjector {
  ...
  async update(
    ...
    for (const terminalTurn of newTerminalTurns) {
      const managedSessionId = findLinkedManagedSessionId(this.application, agentSession);
