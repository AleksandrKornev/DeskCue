...
export function ChangesReview({
  ...
  const visibleDiff = useMemo(() => git
    ? trimUnifiedDiff(filterUnifiedDiff(git.diff))
    : null, [git]);
  const parsedFiles = useMemo(() => parseUnifiedDiff(visibleDiff?.text ?? ""), [visibleDiff?.text]);
  const reviewFiles = useMemo(
    () => mergeDiffReviewFiles(visibleChangedFiles, parsedFiles, git?.changedFileStatuses),
    [git?.changedFileStatuses, parsedFiles, visibleChangedFiles]
  );
        ...
        <section className={styles.changesReview}>
          <header className={styles.changesHeader}>
            <div>
              ...
              <h2>{reviewFiles.length === 1 ? "1 workspace change" : `${reviewFiles.length} workspace changes`}</h2>
          ...
          {!git ? (
            ...
            <div className={styles.emptyState}>
              <strong>{git.isGitRepo ? "Working tree is clean" : "Git is not available here"}</strong>
            ...
            <div className={clsx(styles.changesLayout, reviewingFile && styles.changesLayoutReviewing)}>
              <aside className={styles.changesSidebar}>
                <div className={styles.changesSidebarControls}>
                  ...
                  {hiddenChangedFileCount > 0 ? <p className={styles.sidebarNote}>{hiddenChangedFileCount} generated or temporary files hidden</p> : null}
                </div>
                ...
                {selectedFile ? (
                  <>
                    <header className={styles.diffViewerHeader}>
                      ...
                      {onOpenFile ? <button className={styles.openFileButton} onClick={() => onOpenFile(selectedFile.path)} type="button">Open in Files</button> : null}
                    </header>
                    {visibleDiff?.wasTrimmed ? <p className={styles.diffNotice}>Large diff truncated to keep review responsive.</p> : null}
                    {selectedFile.lines.length > 0 ? (
                      ...
                      <div className={styles.emptyDiff}>
                        ...
                        <p>This is often a new, binary, ignored, or untracked path. Open it in Files to inspect the current contents.</p>
                        {onOpenFile ? <button className={styles.openFileButton} onClick={() => onOpenFile(selectedFile.path)} type="button">Open in Files</button> : null}
                      </div>
                    )}
                  </>
