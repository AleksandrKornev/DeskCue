...
function validateServerEventPayload(
  ...
  switch (type) {
    ...
    case "agent.session.turn.finished":
      ...
      requireOptionalNullableStrings(payload, "answer", "startedAt");
...
function validateAgentSessionSummary(payload: Record<string, unknown>) {
  ...
  if (payload.turnState !== undefined) {
    validateAgentTurnState(payload.turnState);
  }
